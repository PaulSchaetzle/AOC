...
#pragma omp parallel for reduction(+:result) reduction(max:maxScenicScore)
    for (int x = 0; x < forrest[y].size(); x++) {
      if (visibleTree(forrest, x, y)) result++;
      size_t temp = scenicScore(forrest, x, y);
